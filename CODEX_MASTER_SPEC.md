...
#### 当前 Phase 6 检查点（2026-07-14）

1. 已完成 Android debug 工程、arm64 APK、基础月历/编辑保存、通知送达与目标日期导航、通知权限撤销恢复、隐私锁自动安全路径、图片选择/复制/显示/重启持久化和编辑器底栏真机回归。
2. 当前下一项是删除现有图片引用，验证 Markdown、`entry_assets`、原图和缩略图只在无引用时被安全回收；随后重新插入并再次验证重启持久化。
3. 图片闭环后依次执行备份导出、篡改包拒绝、有效恢复与图片/设置一致性，再完成静默日、暂停一周、当天打勾跳过、动态字体、TalkBack、主题、减少动画、小屏、横屏、软键盘和系统返回矩阵。
4. 设备重启会影响当前连接与用户环境，验证 `ReminderBootReceiver` 前必须再次取得用户确认。iOS 真机需要 macOS、Xcode 和 iPhone，当前环境明确阻塞。
