...
2. 主题三态和应用内减少动画已通过；当前设备/WebView 不映射系统减少动画媒体查询，已明确记录为平台限制。200% 字体发现编辑器日期顶栏与工具标签覆盖，最小布局修复已通过前端和 Android APK 构建。
3. 当前下一项是覆盖安装新 APK 并重做 200% 字体真机回归；通过后再继续 TalkBack、小屏、横屏、软键盘、系统返回和安全区矩阵，最后执行设备重启恢复与 iOS 对等验收的可行项。
