...
2. 主题三态和应用内减少动画已通过；当前设备/WebView 不映射系统减少动画媒体查询，已明确记录为平台限制。200% 字体下编辑器日期顶栏、工具标签和横向滚动已通过真机回归。
3. 真机发现 WebView 126 不会在 IME 显示时更新 `visualViewport`，导致编辑器工具栏被键盘覆盖；已用 Android `WindowInsetsCompat.Type.ime()` 向前端提供真实 inset，并与现有 `visualViewport` 偏移取较大值。最终 APK 已覆盖安装，字体 1.0 下的软键盘与系统返回路径通过；200% + IME 组合尚未重测。
4. Gougou 应用验收不授权修改手机全局设置。后续系统字体、TalkBack、旋转和设备重启等设备级操作必须逐项提前取得用户明确同意，并在测试后恢复原值；小屏需要独立设备或模拟器。iOS 真机需要 macOS、Xcode 和 iPhone，当前环境明确阻塞。

#### PHASE 6 结束条件

1. Android 首发矩阵全部通过，iOS 对等能力全部通过或逐项明确阻塞原因。
2. 自动测试、生产前端构建、Android 原生 debug 编译与数据库/备份回归通过。
